...
public interface OrderMapper {
    ...
    //修改菜品订单状态
    Integer updateDishStatus(@Param("dishStatus") Integer dishStatus,@Param("id") Integer id);

    //打印回调(全部)
    Integer printEcho(@Param("orderId") String oderId, @Param("shopId") Integer shopId);

    //打印回调(部分)
    Integer printEchoPart(@Param("dishId")Integer dishId,@Param("orderId") String oderId, @Param("shopId") Integer shopId);

    //通过菜品Id和订单Id获取订单对象信息
    Order selectByOrderIdAndDishId(@Param("dishId")Integer dishId,@Param("orderId") String oderId, @Param("shopId") Integer shopId);
}
