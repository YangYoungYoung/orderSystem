...
//    private String folder = "uploadfiles/";
    ...
    /**
     * 后端接口
     * 根据店铺id获得店铺菜品分类列表跳转添加页面
     * @param shopId 店铺id
     * @param map   类目对象
     * @return
     */
    @GetMapping("/goAddMenu")
    ...
    public ModelAndView addMenu(@Valid MenuForm menu,
        ...
        if (menu.getDishImage()!=null){
            ...
            ossPath = AliOssUtil.uploadFile(inputStream, bucket, endpoint, accessId, accessKey,  menu.getShopId()+"/"+new Date().getTime() + "." + suffix[suffixIndex]);
        }else {
            ossPath=menu.getImage();
        }
        Menu m=new Menu();
        ...
        int a=0;
        try {
            a=Integer.valueOf(menu.getUserType());
        }catch (Exception e){
            e.printStackTrace();
        }
        map.put("userType",a);
    ...
    public ModelAndView updateMenu(@Valid MenuForm menu,
        ...
        int userType=0;
        try {
            userType=Integer.valueOf(menu.getUserType());
        }catch (Exception e){
            e.printStackTrace();
        }
        map.put("userType",userType);
    ...
    /**
     * 后端接口
     * 根据店铺id获得店铺菜品分类列表跳转添加菜品类目列表
     * @param shopId 店铺id
     * @param map   类目对象
     * @return
     */
    @GetMapping("/findDishType")
    public ModelAndView findDishType(@RequestParam("shopId")Integer shopId,
                                     @RequestParam("userType")Integer userType,
                              Map<String,Object> map){
        List<DishType> TypeList= menuService.getDishType(shopId);
        map.put("userType",userType);
        map.put("shopId",shopId);
        map.put("TypeList",TypeList);
        return new ModelAndView("/foodcategory/list",map);
    }


    /**
     * 后端接口
     * 根据菜品类目id跳转到修改店铺菜品类目信息页面
     * @param dishTypeId 菜品类目ID
     * @param map   类目对象
     * @return
     */

    @GetMapping("/goUpdateType" )
    public ModelAndView updateType(@RequestParam("shopId")Integer shopId,
                                   @RequestParam("userType")Integer userType,
                                   @RequestParam("dishTypeId")Integer dishTypeId,
                                   Map<String, Object>map){
        DishType dishType=menuService.findOneType(dishTypeId);
        map.put("userType",userType);
        map.put("shopId" , shopId);
        map.put("dishType" , dishType);
        return new ModelAndView("/foodcategory/index",map);
    }
    ...
    public ModelAndView updateType(@Valid DishType dishType,
                                   Map<String, Object> map){
        menuService.updateDishType(dishType.getSid(),dishType.getId(),dishType.getDishType());
        List<DishType> TypeList= menuService.getDishType(dishType.getSid());
        map.put("userType",dishType.getUserType());
        map.put("shopId",dishType.getSid());
        map.put("TypeList",TypeList);
        return new ModelAndView("/foodcategory/list",map);
    }
    ...
    @GetMapping("/goAddDishType")
    public ModelAndView goAddDishType(@RequestParam("shopId")Integer shopId,
                                      @RequestParam("userType")Integer userType,
                                      Map<String, Object> map){
        map.put("userType",userType);
        map.put("shopId" , shopId);
        return new ModelAndView("/foodcategory/addDishType",map);
    }
    ...
    public ModelAndView addDishType(@Valid DishType dishType,
                                    Map<String, Object> map){
        menuService.addDishType(dishType.getSid(),dishType.getDishType());
        List<DishType> TypeList= menuService.getDishType(dishType.getSid());
        map.put("userType",dishType.getUserType());
        map.put("shopId",dishType.getSid());
        map.put("TypeList",TypeList);
