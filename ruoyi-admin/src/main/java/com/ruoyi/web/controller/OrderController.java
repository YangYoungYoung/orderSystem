...
public class OrderController {
    ...
    public ModelAndView list(@RequestParam("shopId") Integer shopId,
        ...
        List<Order> orderList=orderservice.findAll(shopId);
   ...
   @PostMapping("/updateDishStatus")
    public ModelAndView updateTableStatus(@Valid StatusForm statusForm,
                                          Map<String,Object>map){
        Integer result=orderservice.changeStatusByOrderIdAndDishId(statusForm.getDishStatus(),statusForm.getShopId(),statusForm.getDishId(),statusForm.getOrderId());
        if (result>0){
            OrderDTO orderDTO=new OrderDTO();
            List<Order> orderList=orderservice.selectByOrderId(statusForm.getOrderId(),statusForm.getShopId());
            Double orderAmount = 0.0;
            for (int i = 0; i < orderList.size(); i++) {
                Double price = orderList.get(i).getTotalPrice();
                orderAmount += price;
    ...
    public Result changeStatusByOrderIdAndDishId(HttpServletRequest request) {
        ...
        if(status==3) {
            ...
            //todo
            Order order=orderMapper.selectByOrderIdAndDishId(dishId,orderId,shopId);
            List<Order> list=new ArrayList<>();
            list.add(order);
            printerController.SendMsgToReceipt(list,orderId,shopId,order.getTableId(),2);
        }
        if (rows > 0) {
            return new Result(200, null, "订单状态修改成功");
        } else {
    ...
    public Result updateStatusAndPayTime(HttpServletRequest request) {
        ...
        if (rows > 0) {
            //todo
            printerController.SendMsgToReceiptAll(request);
